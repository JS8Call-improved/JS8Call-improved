...
FrequencyList_v2::FrequencyItems const default_frequency_list = {
    { 1'843'500,   Modes::JS8, IARURegions::ALL }, // 3.5 above
    { 3'578'000,   Modes::JS8, IARURegions::ALL }, // 5 above
    { 5'363'000,   Modes::JS8, IARURegions::ALL }, // not known at this time where FT8 will be
    { 7'078'000,   Modes::JS8, IARURegions::ALL }, // 4 above
    { 10'130'000,  Modes::JS8, IARURegions::ALL }, // 6 below
    { 14'078'000,  Modes::JS8, IARURegions::ALL }, // 4 above
    { 18'104'000,  Modes::JS8, IARURegions::ALL }, // 4 above
    { 21'078'000,  Modes::JS8, IARURegions::ALL }, // 4 above
    { 24'922'000,  Modes::JS8, IARURegions::ALL }, // 9 above
    { 28'078'000,  Modes::JS8, IARURegions::ALL }, // 4 above
    { 50'318'000,  Modes::JS8, IARURegions::ALL }, // 5 above
    { 144'178'000, Modes::JS8, IARURegions::ALL }, // 4 above
};
}
...
QDebug operator<<(QDebug debug, FrequencyList_v2::Item const& item)
{
    ...
    debug.nospace() << "FrequencyItem(" << item.frequency_ << ", " << item.region_ << ", "
                    << item.mode_ << ')';
    return debug;
}
#endif
...
auto FrequencyList_v2::frequency_list(QModelIndexList const& model_index_list) const
    -> FrequencyItems
{
    FrequencyItems list;
...
QVariant FrequencyList_v2::impl::data(QModelIndex const& index, int role) const
{
    ...
    if (index.isValid() && row < frequency_list_.size() && column < num_cols) {
        ...
        case region_column:
            ...
            case Qt::DisplayRole:
            case Qt::EditRole:
            case Qt::AccessibleTextRole: item = IARURegions::name(frequency_item.region_); break;
        ...
        case mode_column:
            ...
            case Qt::DisplayRole:
            case Qt::EditRole:
            case Qt::AccessibleTextRole: item = Modes::name(frequency_item.mode_); break;

            case Qt::ToolTipRole:
            case Qt::AccessibleDescriptionRole: item = tr("Mode"); break;
        ...
        case frequency_column:
            ...
            case SortRole:
            case Qt::EditRole:
            case Qt::AccessibleTextRole: item = frequency_item.frequency_; break;

            case Qt::DisplayRole:
            {
                auto const& band = bands_->find(frequency_item.frequency_);
                item = Radio::pretty_frequency_MHz_string(frequency_item.frequency_) + " MHz ("
                    + (band.isEmpty() ? "OOB" : band) + ')';
            } break;

            case Qt::ToolTipRole:
            case Qt::AccessibleDescriptionRole: item = tr("Frequency"); break;
        ...
        case frequency_mhz_column:
            ...
            case Qt::ToolTipRole:
            case Qt::AccessibleDescriptionRole: item = tr("Frequency (MHz)"); break;
                                     ...
                                     int role)
{
    ...
    if (model_index.isValid() && Qt::EditRole == role && row < frequency_list_.size()) {
        QVector<int> roles;
        roles << role;
...
QVariant
    FrequencyList_v2::impl::headerData(int section, Qt::Orientation orientation, int role) const
{
    QVariant header;
    if (Qt::DisplayRole == role && Qt::Horizontal == orientation && section < num_cols) {
        switch (section) {
...
auto FrequencyList_v2::all_bands(Region region, Mode mode) const -> BandSet
{
    ...
    for (auto const& item : m_->frequency_list_) {
        if (region == IARURegions::ALL || item.region_ == region || mode == Modes::ALL
            || item.mode_ == mode) {
            result << m_->bands_->find(item.frequency_);
        }
    }

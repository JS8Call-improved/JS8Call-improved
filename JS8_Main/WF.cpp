...
Colours load_palette(QString const& file_name)
{
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        while (!in.atEnd()) {
            ...
            if (++count >= points) {
                throw_qstring(QObject::tr("Error reading waterfall palette "
                                          "file \"%1:%2\" too many colors.")
                                  .arg(file.fileName())
                                  .arg(line_counter));
            }
            ...
            if (!r_ok || !g_ok || !b_ok || r < 0 || r > 255 || g < 0 || g > 255 || b < 0
                || b > 255) {
                throw_qstring(QObject::tr("Error reading waterfall palette "
                                          "file \"%1:%2\" invalid color.")
                                  .arg(file.fileName())
                                  .arg(line_counter));
            }
        ...
        throw_qstring(QObject::tr("Error opening waterfall palette file \"%1\": %2.")
                          .arg(file.fileName())
...
public:
    explicit Designer(Colours const& current, QWidget* parent = nullptr) :
        QDialog { parent }, colours_ { current }
    {
        ui_.setupUi(this);
        ...
        connect(ui_.colour_table_widget,
                &QWidget::customContextMenuRequested,
                this,
                &Designer::context_menu);
...
private:
    ...
    void insert_new_item(int row, QColor const& default_colour)
    {
        ...
        auto new_colour
            = QColorDialog::getColor(row > 0 ? colours_[row - 1] : default_colour, this);
        if (new_colour.isValid()) {
    ...
    void export_palette()
    {
        ...
        if (!file_name.isEmpty()) {
            ...
            if (file.open(QFile::WriteOnly | QFile::Truncate | QFile::Text)) {
                ...
                throw_qstring(QObject::tr("Error writing waterfall palette file \"%1\": %2.")
                                  .arg(file.fileName())
                                  .arg(file.errorString()));
            }
        }
...
namespace WF
{
Palette::Palette(QString const& file_path) : colours_ { load_palette(file_path) }
{
}

Palette::Palette(Colours const& colour_list) : colours_ { colour_list }
{
}
...
QVector<QColor> Palette::interpolate() const
{
    ...
    for (int i { 0 }; i < points; ++i) {
        ...
        // qCDebug (wf_js8) << "Palette::interpolate: prior:" << prior <<
        // "total:" << colours.size ();

        auto increment = i - qreal(interval) * prior;
        qreal r { colours[prior].redF()
                  + (increment * (colours[next].redF() - colours[prior].redF())) / interval };
        qreal g { colours[prior].greenF()
                  + (increment * (colours[next].greenF() - colours[prior].greenF())) / interval };
        qreal b { colours[prior].blueF()
                  + (increment * (colours[next].blueF() - colours[prior].blueF())) / interval };
        result.append(QColor::fromRgbF(r, g, b));

        // qCDebug (wf_js8) << "Palette colour[" << (result.size () - 1) << "]
        // =" << result[result.size () - 1] << "from: r:" << r << "g:" << g <<
        // "b:" << b;
    }
...
bool Palette::design()
{
    if (auto designer = Designer { colours_ }; designer.exec() == QDialog::Accepted) {

...
char const *multi_settings_current_group_key =
    "CurrentMultiSettingsConfiguration";
...
//
class NameDialog final : public QDialog {
    Q_OBJECT
  ...
  public:
    explicit NameDialog(QString const &current_name,
        ...
        auto name_validator = new QRegularExpressionValidator{
            QRegularExpression{R"([^/\\]+)"}, this};
        name_line_edit_.setValidator(name_validator);

        connect(&name_line_edit_, &QLineEdit::textChanged,
                [current_names, button_box](QString const &name) {
    ...
    QString new_name() const { return name_line_edit_.text().trimmed(); }
...
//
class ExistingNameDialog final : public QDialog {
    Q_OBJECT

  public:
    explicit ExistingNameDialog(QStringList const &current_names,
                                QWidget *parent = nullptr)
        : QDialog{parent} {
        ...
        auto form_layout = new QFormLayout();
        form_layout->addRow(tr("&Source Configuration Name:"),
                            &name_combo_box_);
    ...
    QString name() const { return name_combo_box_.currentText(); }
...
class MultiSettings::impl final : public QObject {
    Q_OBJECT
  ...
  private:
    ...
    QMenu *create_sub_menu(QMenu *parent, QString const &menu_title,
...
QSettings *MultiSettings::settings() { return &m_->settings_; }

QVariant MultiSettings::common_value(QString const &key,
...
QString MultiSettings::configuration_name() const { return m_->current_; }

bool MultiSettings::exit() { return m_->exit(); }

MultiSettings::impl::impl(MultiSettings const *parent,
                          QString const &config_name)
    : settings_{settings_path(), QSettings::IniFormat}, parent_{parent},
      main_window_{nullptr}, name_change_emit_pending_{true},
      reposition_type_{RepositionType::unchanged}, exit_flag_{true},
    ...
    if (settings_.contains(multi_settings_current_group_key)) {
        ...
        if (current_.size()) {
            {
                SettingsGroup alternatives{&settings_,
                                           multi_settings_root_group};
                {
                    SettingsGroup source_group{&settings_, current_};
                    new_settings_ = get_settings();
                }
                settings_.setValue(multi_settings_current_name_key,
                                   tr(default_string));
            }
    ...
    if (config_name.size() && available_configurations.contains(config_name) &&
        config_name != current_) {
...
bool MultiSettings::impl::reposition() {
    ...
    case RepositionType::save_and_replace: {
        ...
        auto const &previous_group_name =
            settings_.value(multi_settings_current_name_key, tr(default_string))
                .toString();
    ...
    case RepositionType::replace:
        ...
        if (!new_settings_.size()) {
            ...
            qApp->setStyleSheet(qApp->styleSheet() + "* {" +
                                font_as_stylesheet(original_font_) + '}');
        }
    ...
    if (current_group.size())
        settings_.beginGroup(current_group);
...
// and, reset
void MultiSettings::impl::create_menu_actions(QMainWindow *main_window,
                                              QMenu *menu) {
    ...
    SettingsGroup alternatives{&settings_, multi_settings_root_group};
    // get the current configuration name
    auto const &current_configuration_name =
        settings_.value(multi_settings_current_name_key, tr(default_string))
            .toString();
    // add the default configuration sub menu
    QMenu *default_menu = create_sub_menu(menu, current_configuration_name,
                                          configurations_group_);
...
QMenu *MultiSettings::impl::create_sub_menu(QMenu *parent_menu,
    ...
    if (action_group)
        action_group->addAction(sub_menu->menuAction());
    ...
    connect(sub_menu, &QMenu::aboutToShow, [this, parent_menu, sub_menu]() {
        ...
        connect(clone_action, &QAction::triggered,
                [this, parent_menu, sub_menu](bool) {
        ...
        if (settings_.childGroups().size()) {
            ...
            connect(
                clone_into_action, &QAction::triggered,
                [this, sub_menu](bool) { clone_into_configuration(sub_menu); });
        }
        if (current_group.size())
            settings_.beginGroup(current_group);
...
void MultiSettings::impl::select_configuration(QString const &target_name) {
    if (main_window_ && target_name != current_) {
        ...
        if (changed) {
            ...
            Q_EMIT parent_->configurationNameChanged(
                unescape_ampersands(current_));
...
void MultiSettings::impl::clone_configuration(QMenu *parent_menu,
    ...
    do {
        if (index++)
            new_name = new_name_root + '(' + QString::number(index) + ')';
...
void MultiSettings::impl::clone_into_configuration(QMenu const *menu) {
    ...
    QStringList sources;
    {
        SettingsGroup alternatives{&settings_, multi_settings_root_group};
        current_group_name =
            settings_.value(multi_settings_current_name_key).toString();
    ...
    if (sources.size() &&
        (1 == sources.size() || QDialog::Accepted == dialog.exec())) {
        QString source_name{1 == sources.size() ? sources.at(0)
                                                : dialog.name()};
        if (main_window_ &&
            JS8MessageBox::Yes ==
                JS8MessageBox::query_message(
                    main_window_, tr("Clone Into Configuration"),
                    tr("Confirm overwrite of all values for configuration "
                       "\"%1\" with values from \"%2\"?")
...
void MultiSettings::impl::reset_configuration(QMenu const *menu) {
    ...
    if (!main_window_ ||
        JS8MessageBox::Yes !=
            JS8MessageBox::query_message(
                main_window_, tr("Reset Configuration"),
...
void MultiSettings::impl::delete_configuration(QMenu *menu) {
    ...
    if (target_name == current_) {
        ...
        if (!main_window_ ||
            JS8MessageBox::Yes !=
                JS8MessageBox::query_message(
                    main_window_, tr("Delete Configuration"),

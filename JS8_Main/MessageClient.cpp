...
public:
    ...
    impl(quint16 const port, MessageClient* self) :
        self_ { self }, port_ { port }, ping_ { new QTimer { this } }
    {
        ...
        connect(this, &QIODevice::readyRead, this, [this]() {
            while (hasPendingDatagrams()) {
                if (auto const datagram = receiveDatagram(); datagram.isValid()) {
                    try {
                        Q_EMIT self_->message(Message::fromJson(datagram.data()));
                    } catch (std::exception const& e) {
                        Q_EMIT self_->error(
                            QString { "MessageClient exception: %1" }.arg(e.what()));
                    }
                }
            }
        });
        ...
        if (port_ && !host_.isNull()) {
            send_message({
                ...
                { { "NAME", QVariant(QApplication::applicationName()) },
                  ...
                  { "UTC",
                    QVariant(DriftingDateTime::currentDateTimeUtc().toMSecsSinceEpoch()) } }
            });
        }
    }
    ...
    void send_message(Message const& message)
    {
        ...
        if (auto const datagram = message.toJson(); datagram != lastDatagram_) {
    ...
    void queue_host_lookup(QString const& name)
    {
        ...
        hostLookupId_ = QHostInfo::lookupHost(name, this, [this](QHostInfo const& info) {
            // This functor is always called in the context of the thread
            // that made the call to lookupHost(), so we're safe to modify
            // anything that we were safe to modify outside.
            ...
            if (info.lookupId() == hostLookupId_) {
                ...
                if (auto const& list = info.addresses(); !list.isEmpty()) {
                    ...
                    qCDebug(messageclient_js8)
                        << "MessageClient Host:" << host_.toString()
                        << "loopback:" << host_.isLoopback() << "multicast:" << host_.isMulticast();
...
MessageClient::MessageClient(QString const& name, quint16 const port, QObject* parent) :
...
QHostAddress MessageClient::server_host() const
{
    return m_->host_;
}
...
quint16 MessageClient::server_port() const
{
    return m_->port_;
}
...
void MessageClient::set_server_port(quint16 const port)
{
    m_->port_ = port;
}

...
public:
    ...
    bool has_CAT_indirect_serial_PTT(
        QString const& name) const; // Can PTT via CAT port use DTR or RTS (OmniRig for example)
    ...
    struct ParameterPack
    {
        ...
        bool operator==(ParameterPack const& rhs) const
        {
            return rhs.rig_name == rig_name && rhs.serial_port == serial_port
                && rhs.network_port == network_port && rhs.usb_port == usb_port && rhs.baud == baud
                ...
                && rhs.audio_source == audio_source && rhs.split_mode == split_mode
                && rhs.ptt_port == ptt_port && rhs.poll_interval == poll_interval;
        }
    };

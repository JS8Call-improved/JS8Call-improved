...
void TxLoop::onTimer()
{
    ...
    if (m_active) {
        ...
        if (0 != into_mode_slot)
            qCWarning(txloop_js8) << m_name << "in onTimer callback, next activity planned for"
        ...
        if (next_activity_ms - m_tx_delay_ms < now_ms) {
            qint64 loop_periods_missing
                = (now_ms - (next_activity_ms - m_tx_delay_ms)) / m_loop_period_ms + 1;
            qint64 additional_increase = loop_periods_missing * m_loop_period_ms;
            qCWarning(txloop_js8) << m_name << "next activity planned for"
        ...
        qCDebug(txloop_js8) << m_name << "triggers TX, and also plans future signal to happen at"
                            << m_next_activity << ". Taking into account TX delay" << m_tx_delay_ms
                            << "ms, the timer is to wake us in" << wait_for_next_trigger << "ms";
    ...
    if (m_active) {
        qCDebug(txloop_js8) << m_name
                            << "Internal signal plumbing completed. "
                               "Telling all who want to know that the next transmission has been "
                               "scheduled to occur at"
                            << m_next_activity;
        emit nextActivityChanged(m_next_activity);
    } else {
        qCDebug(txloop_js8) << m_name
                            << "Internal signal plumbing completed.  Telling all who want to know "
                               "I'm idle with not transmission planned.";
        emit canceled();
    }
}
...
void TxLoop::onDriftChange(qint64 /* new_drift */)
{
    if (m_active) {
        ...
        if (now < need_to_push_ptt && need_to_push_ptt < now.addMSecs(m_loop_period_ms)) {
            ...
            qCDebug(txloop_js8) << m_name << "Small drift change, pushing PTT in" << fire_in_ms
                                << "ms";
            m_next_activity_timer.start(fire_in_ms);
        } else {
            qCDebug(txloop_js8)
                << m_name
                << "After big drift change, need to restart transmission schedule from square one.";
...
void TxLoop::onModeChange(Varicode::SubmodeType new_submode)
{
    if (m_submode != new_submode) {
        qCDebug(txloop_js8) << m_name << "Submode change from" << JS8::Submode::name(m_submode)
        ...
        if (m_loop_period_ms < submode_period_ms)
            throw std::invalid_argument { boost::str(
                boost::format("Submode period of %1%ms does not fit tx schedule period of %2%ms.")
...
{
    qCDebug(txloop_js8) << m_name << "TX delay change from" << this->m_tx_delay_ms << "to"
                        << tx_delay_ms;
    ...
    if (tx_delay_ms < 0)
        throw std::invalid_argument { boost::str(
            boost::format("Negative tx delay of %1%ms not acceptable.") % tx_delay_ms) };
...
void TxLoop::onLoopCancel()
{
    if (m_active) {
        ...
        m_next_activity_timer.stop();
        qCDebug(txloop_js8)
            << m_name << "Canceling scheduled TX activity (and telling all who want to know).";
        emit canceled();
    } else {
        qCDebug(txloop_js8) << m_name
...
void TxLoop::onTxLoopPeriodChangeStart(qint64 loop_period_ms)
{
    ...
    qint64 next_start = 0 == remainder ? earliest : earliest + mode_period_ms - remainder;
    bool need_to_send_signal = !m_active || next_start != m_next_activity.currentMSecsSinceEpoch();
    ...
    qCDebug(txloop_js8) << m_name << "Active tx loop, transmission to start at" << m_next_activity
                        << (need_to_send_signal ?
                                "newly scheduled, so telling all who want to know." :
                                "but this is nothing new.")
                        << "Need to become busy in" << need_to_wait << "ms for tx_delay_ms"
                        << m_tx_delay_ms << "loop_period_ms" << m_loop_period_ms << "mode_period_ms"
                        << mode_period_ms;
    // << "now_ms" << now_ms
    // << "earliest" << earliest
    // << "remainder" << remainder;

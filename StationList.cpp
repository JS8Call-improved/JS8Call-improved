...
QDebug operator<<(QDebug debug, StationList::Station const& station)
{
    ...
    debug.nospace() << "Station(" << station.band_name_ << ", " << station.frequency_ << ", "
                    << station.switch_at_ << ", " << station.switch_until_ << ", "
...
{
    is >> station.band_name_ >> station.frequency_ >> station.switch_at_ >> station.switch_until_
        >> station.description_;
...
public:
    ...
    bool dropMimeData(QMimeData const*,
                      Qt::DropAction,
                      int row,
                      int column,
    ...
    QModelIndex first_matching_band(QString const& band_name) const
    {
        ...
        auto matches
            = bands_->match(bands_->index(0, 0), Qt::DisplayRole, band_name, 1, Qt::MatchExactly);
...
StationList::StationList(Bands const* bands, QObject* parent) : StationList { bands, {}, parent }
{
}
...
QVariant StationList::impl::data(QModelIndex const& index, int role) const
{
    ...
    if (index.isValid() && row < stations_.size()) {
        ...
        case band_column:
            ...
            case Qt::DisplayRole:
            case Qt::EditRole:
            case Qt::AccessibleTextRole: item = stations_.at(row).band_name_; break;

            case Qt::ToolTipRole:
            case Qt::AccessibleDescriptionRole: item = tr("Band name"); break;
        ...
        case frequency_column:
        {
            ...
            case SortRole:
            case Qt::EditRole:
            case Qt::AccessibleTextRole: item = frequency_offset; break;
        ...
        case switch_at_column:
            ...
            case Qt::ToolTipRole:
            case Qt::AccessibleDescriptionRole: item = tr("Switch at this time"); break;
        ...
        case switch_until_column:
            ...
            case Qt::AccessibleDescriptionRole:
                item = tr("Switch until this time\nFrequency will not return at end time.\nEnter a "
                          "new line to return to original or other frequency.");
                break;
        ...
        case description_column:
            ...
            case Qt::EditRole:
            case Qt::DisplayRole:
            case Qt::AccessibleTextRole: item = stations_.at(row).description_; break;

            case Qt::ToolTipRole:
            case Qt::AccessibleDescriptionRole: item = tr("Antenna description"); break;
...
bool StationList::impl::setData(QModelIndex const& model_index, QVariant const& value, int role)
{
    ...
    if (model_index.isValid() && Qt::EditRole == role && row < size) {
        ...
        case switch_at_column:
        {
            ...
            auto switch_until_index
                = model_index.model()->index(row, switch_until_column, model_index);
                                     ...
                                     QModelIndex const& parent)
{
    ...
    if (parent.isValid() && description_column == parent.column() && data->hasFormat(mime_type)) {
        ...
        while (!stream.atEnd()) {
            ...
            if (stations_.cend()
                == std::find_if(stations_.cbegin(), stations_.cend(), [&band](Station const& s) {

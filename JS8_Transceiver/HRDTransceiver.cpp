...
{
    (*registry)[HRD_transceiver_name]
        = TransceiverFactory::Capabilities(id,
                                           TransceiverFactory::Capabilities::network,
                                           true,
                                           true /* maybe */);
}
...
int HRDTransceiver::do_start()
{
    ...
    Q_FOREACH (auto const& s, slider_names_) {
        auto range = send_command("get slider-range " + current_radio_name + " " + s)
                         .trimmed()
                         .split(',', Qt::SkipEmptyParts);
    ...
    if ((split_mode_dropdown_ = find_dropdown(QRegularExpression("^(Split)$"))) >= 0) {
        split_mode_dropdown_selection_on_
            = find_dropdown_selection(split_mode_dropdown_, QRegularExpression("^(On)$"));
        split_mode_dropdown_selection_off_
            = find_dropdown_selection(split_mode_dropdown_, QRegularExpression("^(Off)$"));
    } else if ((receiver_dropdown_ = find_dropdown(QRegularExpression("^Receiver$"))) >= 0) {
        rx_A_selection_
            = find_dropdown_selection(receiver_dropdown_, QRegularExpression("^(RX / Off)$"));
        rx_B_selection_
            = find_dropdown_selection(receiver_dropdown_, QRegularExpression("^(Mute / RX)$"));
    }
    ...
    if ((data_mode_dropdown_ = find_dropdown(QRegularExpression("^(Data)$"))) >= 0) {
        data_mode_dropdown_selection_on_
            = find_dropdown_selection(data_mode_dropdown_,
                                      QRegularExpression("^(On|Data1|D1|D1-FIL1)$"));
        data_mode_dropdown_selection_off_
            = find_dropdown_selection(data_mode_dropdown_, QRegularExpression("^(Off)$"));
    }
    ...
    if (vfo_count_ == 1
        && ((vfo_B_button_ >= 0 && vfo_A_button_ >= 0) || vfo_toggle_button_ >= 0)) {
...
std::vector<int> HRDTransceiver::find_dropdown_selection(int dropdown,
...
void HRDTransceiver::map_modes(int dropdown, ModeMap* map)
{
    ...
    map->push_back(std::forward_as_tuple(
        CW,
        find_dropdown_selection(dropdown, QRegularExpression("^(CW|CW\\(N\\))|CWL$"))));
    map->push_back(std::forward_as_tuple(
        CW_R,
        find_dropdown_selection(dropdown, QRegularExpression("^(CW-R|CW-R\\(N\\)|CW|CWU)$"))));
    map->push_back(std::forward_as_tuple(
        LSB,
        find_dropdown_selection(dropdown, QRegularExpression("^(LSB\\(N\\)|LSB)$"))));
    map->push_back(std::forward_as_tuple(
        USB,
        find_dropdown_selection(dropdown, QRegularExpression("^(USB\\(N\\)|USB)$"))));
    map->push_back(std::forward_as_tuple(
        DIG_U,
        find_dropdown_selection(dropdown,
                                QRegularExpression("^(DIG|DIGU|DATA-U|PKT-U|DATA|USER-U|USB)$"))));
    map->push_back(std::forward_as_tuple(
        DIG_L,
        find_dropdown_selection(
            dropdown,
            QRegularExpression("^(DIG|DIGL|DATA-L|PKT-L|DATA-R|USER-L|LSB)$"))));
    map->push_back(std::forward_as_tuple(
        FSK,
        find_dropdown_selection(dropdown, QRegularExpression("^(DIG|FSK|RTTY|RTTY-LSB)$"))));
    map->push_back(std::forward_as_tuple(
        FSK_R,
        find_dropdown_selection(dropdown,
    ...
    map->push_back(std::forward_as_tuple(
        AM,
        find_dropdown_selection(dropdown, QRegularExpression("^(AM|DSB|SAM|DRM)$"))));
    map->push_back(std::forward_as_tuple(
        FM,
        find_dropdown_selection(dropdown, QRegularExpression("^(FM|FM\\(N\\)|FM-N|WFM)$"))));
    map->push_back(std::forward_as_tuple(
        DIG_FM,
        find_dropdown_selection(dropdown, QRegularExpression("^(PKT-FM|PKT|FM)$"))));
    ...
    std::for_each(map->begin(), map->end(), [this, dropdown](ModeMap::value_type const& item) {
        ...
        TRACE_CAT(
            "HRDTransceiver",
            '\t' << std::get<0>(item) << "<->"
                 << (rhs.size() ? dropdowns_[dropdown_names_[dropdown]][rhs.front()] : "None"));
    });
}
...
void HRDTransceiver::set_data_mode(MODE m)
{
    if (data_mode_toggle_button_ >= 0) {
        ...
        case DIG_U:
        case DIG_L:
        case DIG_FM: set_button(data_mode_toggle_button_, true); break;
        default: set_button(data_mode_toggle_button_, false); break;
        }
    } else if (data_mode_on_button_ >= 0 && data_mode_off_button_ >= 0) {
        switch (m) {
        case DIG_U:
        case DIG_L:
        case DIG_FM: set_button(data_mode_on_button_, true); break;
        default: set_button(data_mode_off_button_, true); break;
        }
    } else if (data_mode_dropdown_ >= 0 && data_mode_dropdown_selection_off_.size()
...
void HRDTransceiver::do_frequency(Frequency f, MODE m, bool /*no_ignore*/)
{
    ...
    if (vfo_count_ > 1 && reversed_) {
        ...
        send_simple_command("set frequencies-hz " + QString::number(frequencies[0].toUInt()) + ' '
                            + fo_string);
    } else {
        send_simple_command("set frequency-hz " + QString::number(f));
    }
...
void HRDTransceiver::do_tx_frequency(Frequency tx, MODE mode, bool /*no_ignore*/)
{
    ...
    if (split) {
        if (mode != UNK) {
            if (!reversed_ && mode_B_dropdown_ >= 0) {
                ...
                Q_ASSERT(
                    mode_A_dropdown_ >= 0
                    && ((vfo_A_button_ >= 0 && vfo_B_button_ >= 0) || vfo_toggle_button_ >= 0));
                ...
                if (rx_B_button_ >= 0) {
                    ...
                    set_dropdown(receiver_dropdown_,
                                 (reversed_ ? rx_A_selection_ : rx_B_selection_).front());
                    set_dropdown(mode_A_dropdown_, lookup_mode(mode, mode_A_map_));
                    set_data_mode(mode);
                    set_dropdown(receiver_dropdown_,
                                 (reversed_ ? rx_B_selection_ : rx_A_selection_).front());
                } else if (vfo_count_ > 1
                           && ((vfo_A_button_ >= 0 && vfo_B_button_ >= 0)
                               || vfo_toggle_button_ >= 0)) {
                    set_button(vfo_A_button_ >= 0 ? (reversed_ ? vfo_A_button_ : vfo_B_button_) :
                                                    vfo_toggle_button_);
                    set_dropdown(mode_A_dropdown_, lookup_mode(mode, mode_A_map_));
                    set_data_mode(mode);
                    set_button(vfo_A_button_ >= 0 ? (reversed_ ? vfo_B_button_ : vfo_A_button_) :
                                                    vfo_toggle_button_);
                }
        ...
        if (reversed_) {
            ...
            auto frequencies
                = send_command("get frequencies").trimmed().split('-', Qt::SkipEmptyParts);
    ...
    } else if (split_mode_dropdown_ >= 0 && split_mode_dropdown_selection_off_.size()
...
void HRDTransceiver::poll()
{
    ...
    if (vfo_count_ > 1) {
        auto frequencies
            = send_command("get frequencies", quiet).trimmed().split('-', Qt::SkipEmptyParts);
    ...
    if (vfo_count_ > 1 || !state().ptt()) {
        update_mode(
            get_data_mode(lookup_mode(get_dropdown(mode_A_dropdown_, quiet), mode_A_map_), quiet));
    }
}

QString HRDTransceiver::send_command(QString const& cmd,
                                     bool no_debug,
                                     bool prepend_context,
                                     bool recurse)
{
    Q_ASSERT(hrd_);
    ...
    if (!recurse && prepend_context) {
        ...
        auto radio_iter = std::find_if(radios_.begin(),
                                       radios_.end(),
    ...
    if (v4 == protocol_) {
        ...
        if (!write_to_port(message.constData(), message.size())) {
            ...
            throw error { tr("Ham Radio Deluxe: failed to write command \"%1\"").arg(cmd) };
        }
    } else {
...
QByteArray HRDTransceiver::read_reply(QString const& cmd)
{
    ...
    if (!replied) {
        ...
        throw error { tr("Ham Radio Deluxe retries exhausted sending command \"%1\"").arg(cmd) };
    }
    return hrd_->readAll();
}
...
void HRDTransceiver::send_simple_command(QString const& command, bool no_debug)
{
    if ("OK" != send_command(command, no_debug)) {
        ...
        throw error {
            tr("Ham Radio Deluxe didn't respond to command \"%1\" as expected").arg(command)
        };
    }
}

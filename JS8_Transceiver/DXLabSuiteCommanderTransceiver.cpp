...
QString map_mode(Transceiver::MODE mode)
{
    ...
    case Transceiver::FM:
    case Transceiver::DIG_FM: return "FM";
    default: break;
    }
    return "USB";
}
...
void DXLabSuiteCommanderTransceiver::register_transceivers(
    ...
    int id)
{
    (*registry)[commander_transceiver_name]
        = TransceiverFactory::Capabilities { id, TransceiverFactory::Capabilities::network, true };
}
...
int DXLabSuiteCommanderTransceiver::do_start()
{
    ...
    auto server_details = network_server_lookup(server_,
                                                52002u,
        ...
        if (f && !(f % 10)) {
            ...
            if (0 == reply.indexOf("<CmdFreq:")) {
                ...
                if (1 == resolution) // may be 20Hz rounded
                {
                    ...
                    simple_command(
                        ("<command:10>CmdSetFreq<parameters:%1>" + params).arg(params.size()));
                    QThread::msleep(2000);
                    reply = command_with_reply("<command:10>CmdGetFreq<parameters:0>");
...
void DXLabSuiteCommanderTransceiver::do_ptt(bool on)
{
    ...
    if (use_for_ptt_) {
        ...
        if (tx != on) {
            ...
            throw error { tr("DX Lab Suite Commander rig did not respond to PTT: ")
                          + (on ? "ON" : "OFF") };
        }
    } else {
        Q_ASSERT(wrapped_);
...
QString DXLabSuiteCommanderTransceiver::command_with_reply(QString const& cmd, bool no_debug)
{
    ...
    if (!write_to_port(cmd)) {
        ...
        throw error { tr("DX Lab Suite Commander failed to send command \"%1\": %2\n")
                          .arg(cmd)
    ...
    while (!replied && --retries) {
        ...
        if (!replied && commander_->error() != commander_->SocketTimeoutError) {
            TRACE_CAT("DXLabSuiteCommanderTransceiver",
                      cmd << "failed to read reply:" << commander_->errorString());
...
Q_LOGGING_CATEGORY(dxlabsuitecommandertransceiver_js8,
                   "dxlabsuitecommandertransceiver.js8",
                   QtWarningMsg)

...
}; // alphabet to encode _into_ for FT8 freetext transmission
...
QMap<QString, int> directed_cmds = {
    ...
    // ?*^&@
    { " HEARTBEAT",
     -1                    }, // this is my heartbeat (unused except for faux processing of HBs as directed commands)
    { " HB",
     -1                    }, // this is my heartbeat (unused except for faux processing of HBs as directed commands)
...
QString optional_cmd_pattern = QString(
    "(?<cmd>\\s?(?:AGN[?]|QSL[?]|HW CPY[?]|MSG TO[:]|SNR[?]|INFO[?]|GRID[?]|STATUS[?]|QUERY "
    "MSGS[?]|HEARING[?]|(?:(?:STATUS|HEARING|QUERY CALL|QUERY "
    "MSGS|QUERY|CMD|MSG|NACK|ACK|73|YES|NO|HEARTBEAT SNR|SNR|QSL|RR|SK|FB|INFO|GRID|DIT DIT)(?=[ "
    "]|$))|[?> ]))?");
...
QRegularExpression compound_re("^\\s*[`]" + callsign_pattern + "(?<extra>" + optional_grid_pattern
                               + // there's a reason this is first (see: buildMessageFrames)

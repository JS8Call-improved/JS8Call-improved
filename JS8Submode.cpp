...
namespace
{
    ...
    public:
        ...
        constexpr Data(const char* const name,
            ...
            m_samplesNeeded(
                floor(m_samplesForSymbols + (0.5 + startDelayMS / 1000.0) * JS8_RX_SAMPLE_RATE)),
    ...
    constexpr Data Normal = { "NORMAL",        JS8A_SYMBOL_SAMPLES,    JS8A_START_DELAY_MS,
    ...
    constexpr Data Turbo = { "TURBO",
                             ...
                             -20,
                             32 };
    constexpr Data Slow = { "SLOW",          JS8E_SYMBOL_SAMPLES,    JS8E_START_DELAY_MS,
...
{
    return data(submode).period();
}

int rxSNRThreshold(int const submode)
{
    return data(submode).rxSNRThreshold();
}

int rxThreshold(int const submode)
{
    return data(submode).rxThreshold();
}
...
{
    return data(submode).toneSpacing();
}

double dataDuration(int const submode)
{
    return data(submode).dataDuration();
}

double txDuration(int const submode)
{
    return data(submode).txDuration();
}
...
int computeAltCycleForDecode(int const submode, int const k, int const offsetFrames)
{
    ...
    return computeCycleForDecode(submode, altK < 0 ? altK + JS8_RX_SAMPLE_SIZE : altK);
}

...
protected:
    ...
    void store(char const* source, size_t numFrames, qint16* dest)
    {
        ...
        for (qint16 const* i = begin; i != begin + numFrames * (bytesPerFrame() / sizeof(qint16));
             i += bytesPerFrame() / sizeof(qint16)) {
            switch (m_channel) {
            case Mono: *dest++ = *i; break;

            case Right: *dest++ = *(i + 1); break;

            case Both: // should be able to happen but if it
                       // does we'll take left
                Q_ASSERT(Both == m_channel);
                [[fallthrough]];
            case Left: *dest++ = *i; break;
            }
        }
    }

...
public:
    ...
    BroadcastAudioExtension const* bext() const
    {
        return bext_.isEmpty() ?
            nullptr :
            reinterpret_cast<BroadcastAudioExtension const*>(bext_.constData());
    }
        ...
        if (!memcmp(&outer_desc.id_, "RIFF", 4) || be) {
            ...
            if (!memcmp(riff_item, "WAVE", 4)) {
                ...
                while (wave_offset < outer_offset + sizeof outer_desc + outer_size - 1) {
                    if (file_.read(&wave_desc, sizeof wave_desc) != sizeof wave_desc)
                        return false;
                    wave_size = be ? qFromBigEndian<quint32>(wave_desc.size_) :
                    ...
                    if (!memcmp(&wave_desc.id_, "fmt ", 4)) {
                        ...
                        if (file_.read(reinterpret_cast<char*>(&fmt), sizeof fmt) != sizeof fmt)
                            return false;
                        auto audio_format = be ? qFromBigEndian<quint16>(fmt.audio_format) :
                        ...
                        format_.setChannelCount(be ? qFromBigEndian<quint16>(fmt.num_channels) :
                                                     qFromLittleEndian<quint16>(fmt.num_channels));
                        format_.setSampleRate(be ? qFromBigEndian<quint32>(fmt.sample_rate) :
                        ...
                        char list_type[4];
                        if (file_.read(list_type, sizeof list_type) != sizeof list_type)
                            return false;
                        if (!memcmp(list_type, "INFO", 4)) {
                            ...
                            while (info_offset < wave_offset + sizeof wave_desc + wave_size - 1) {
                                if (file_.read(&info_desc, sizeof info_desc) != sizeof info_desc)
                                    return false;
                                info_size = be ? qFromBigEndian<quint32>(info_desc.size_) :
                                                 qFromLittleEndian<quint32>(info_desc.size_);
                    ...
                    if (!file_.seek(wave_offset + sizeof wave_desc + (wave_size + 1) / 2 * 2))
                        return false;
                    wave_offset = file_.pos();
                }
            }
        }
        if (!file_.seek(outer_offset + sizeof outer_desc + (outer_size + 1) / 2 * 2))
            return false;
...
bool BWFFile::impl::write_header(QAudioFormat format)
{
    ...
    if (be) {
        ...
        fmt.byte_rate = qToBigEndian<quint32>(format.bytesForDuration(1'000'000));
        ...
        fmt.byte_rate = qToLittleEndian<quint32>(format.bytesForDuration(1'000'000));
    ...
    if (file_.write(&desc, sizeof desc) != sizeof desc)
        return false;
    if (file_.write(reinterpret_cast<char const*>(&fmt), sizeof fmt) != sizeof fmt)
        return false;
...
bool BWFFile::impl::update_header()
{
    ...
    if (info_dictionary_.size()) {
        ...
        if (file_.write("INFO", 4) != 4)
            return false;
        for (auto iter = info_dictionary_.constBegin(); iter != info_dictionary_.constEnd();
             ++iter) {
            auto value = iter.value();
            auto len = value.size() + 1; // include terminating null char
            desc.set(iter.key().data(),
                     be ? qToBigEndian<quint32>(len) : qToLittleEndian<quint32>(len));
            if ((file_.size() % 2) && file_.write("\0", 1) != 1)
                return false;
            if (file_.write(&desc, sizeof desc) != sizeof desc)
                return false;
    ...
    if ((file_.size() % 2) && file_.seek(file_.size()) && file_.write("\0", 1) != 1)
        return false;
    if (!file_.seek(0))
        return false;
...
//
BWFFile::BWFFile(QAudioFormat const& format, QObject* parent) : QIODevice { parent }, m_ { format }
{
}
...
BWFFile::BWFFile(QAudioFormat const& format,
                 QString const& name,
...
QAudioFormat const& BWFFile::format() const
{
    return m_->format_;
}
...
{
    m_->header_dirty_ = true;
    ::memcpy(m_->bext()->originator_,
             originator.constData(),
...
{
    m_->header_dirty_ = true;
    ::memcpy(m_->bext()->originator_reference_,
             reference.constData(),
...
bool BWFFile::exists() const
{
    return m_->file_.exists();
}
...
void BWFFile::setFileName(QString const& name)
{
    m_->file_.setFileName(name);
}

QString BWFFile::symLinkTarget() const
{
    return m_->file_.symLinkTarget();
}

QString BWFFile::fileName() const
{
    return m_->file_.fileName();
}

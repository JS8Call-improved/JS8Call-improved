...
private:
    ...
    QMenu* create_sub_menu(QMenu* parent, QString const& menu_title, QActionGroup* = nullptr);
...
MultiSettings::MultiSettings(QString const& config_name) : m_ { this, config_name }
{
}
...
bool MultiSettings::impl::reposition()
{
    ...
    case RepositionType::replace:
        ...
        settings_.sync();
        // fall through
    case RepositionType::unchanged: new_settings_.clear(); break;
    }
    if (current_group.size())
...
void MultiSettings::impl::clone_into_configuration(QMenu const* menu)
{
    ...
    if (sources.size() && (1 == sources.size() || QDialog::Accepted == dialog.exec())) {
        ...
        if (main_window_
            && MessageBox::Yes
                == MessageBox::query_message(main_window_,

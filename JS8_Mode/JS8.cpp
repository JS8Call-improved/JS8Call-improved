...
constexpr double cos_approx(double x)
{
    ...
    constexpr auto poly = [](double const x) {
        ...
        return coefficients[0] + coefficients[1] * x2 + coefficients[2] * x4 + coefficients[3] * x6
            + coefficients[4] * x8 + coefficients[5] * x10 + coefficients[6] * x12
...
static_assert(BASELINE_SAMPLE >= 0 && BASELINE_SAMPLE <= 100, "Sample must be a percentage");
...
struct Sync
{
    ...
    Sync(float const freq, float const step, float const sync) : freq(freq), step(step), sync(sync)
    {
    }
};
...
using SyncIndex = MI::multi_index_container<
    ...
    MI::indexed_by<
        ...
        MI::ordered_non_unique<MI::tag<Tag::Sync>, MI::key<&Sync::sync>, std::greater<>>>>;
...
namespace
{
                    ...
                    std::array<int8_t, N>& cw)
    {
        ...
        for (int iter = 0; iter <= BP_MAX_ITERATIONS; ++iter) {
            ...
            for (int i = 0; i < N; ++i) {
                zn[i] = llr[i]
                    + std::accumulate(tov[i].begin(), tov[i].begin() + BP_MAX_CHECKS, 0.0f);
            }
            ...
            for (int i = 0; i < M; ++i) {
                for (int j = 0; j < 7; ++j) { // Fixed range [0, 7) to match Fortran's 1:7, could be
...
namespace
{
    ...
    bool checkCRC12(std::array<std::int8_t, KK> const& decoded)
    {
        ...
        uint16_t crc
            = (static_cast<uint16_t>(bits[9] & 0x1F) << 7) | (static_cast<uint16_t>(bits[10]) >> 1);
    ...
    template <typename Mode> class DecodeMode
    {
        ...
        static constexpr auto Taper = [] {
            ...
            for (size_t i = 0; i <= Mode::NDD; ++i) {
                float const value
                    = 0.5f * (1.0f + cos_approx(i * std::numbers::pi_v<float> / Mode::NDD));
        ...
        std::optional<Decode> js8dec(bool const syncStats,
                                     ...
                                     JS8::Event::Emitter emitEvent)
        {
            ...
            float const dphi
                = -delfbest * ((2.0f * std::numbers::pi_v<float>) / FS2); // Phase increment
            ...
            auto const logTracker = [&](char const* tag) {
                if (decoder_js8().isDebugEnabled()) {
                    if (freqTracker.enabled()) {
                        qCDebug(decoder_js8)
                            << "freqTracker" << tag << "coarseHz" << coarseStartHz << "fineHz" << f1
                    ...
                    if (timingTracker.enabled()) {
                        qCDebug(decoder_js8)
                            << "timingTracker" << tag << "coarseDt" << coarseStartDt << "fineDt"
                            << xdt << "refinedDt"
                            << xdt + static_cast<float>(timingTracker.currentSamples() * DT2)
                            << "avgStepSmpl"
                            << static_cast<float>(timingTracker.averageStepSamples());
                    }
                }
            };

            auto const goertzelEnergy = [&](int start, int expectedTone) -> std::optional<float> {
                ...
                auto const wstep
                    = std::polar(1.0f, static_cast<float>(-TAU * expectedTone / Mode::NDOWNSPS));
                    ...
                    if (isPilot) {
                        ...
                        if (timingTracker.enabled()) {
                            ...
                            if (e0 && eEarly && eLate && toneMag > 1e-6f) {
                                ...
                                double const weight
                                    = std::clamp(static_cast<double>(toneMag / 5.0f), 0.0, 1.0);
                                double const errorSamples
                                    = std::clamp(0.25 * static_cast<double>(grad), -1.0, 1.0);
                ...
                for (std::size_t column = 0; column < 7; ++column) {
                    ...
                    auto const max_row = std::distance(
                        ...
                        std::max_element(
                            s2.begin(),
                            s2.end(),
                            [index = offset + column](auto const& rowA, auto const& rowB) {
            ...
            if (syncStats)
                emitEvent(JS8::Event::SyncState { JS8::Event::SyncState::Type::CANDIDATE,
                                                  Mode::NSUBMODE,
                                                  f1,
                                                  xdt,
                                                  { .candidate = nsync } });
            ...
            auto const whitening
                = js8::WhiteningProcessor<NROWS, ND, N>::process(s1,
                                                                 symbolWinners,
            ...
            if (!whitening.erasureApplied) {
                ...
                if (decoder_js8().isDebugEnabled()) {
                    ...
                    qCDebug(decoder_js8) << "LLR erasure threshold" << m_llrErasureThreshold
            ...
            auto const tryDecode
                = [&](std::array<float, N> const& llrInput, int ipass) -> std::optional<Decode> {
                ...
                if (nharderrors >= 0 && nharderrors < 60 && !(sync < 2.0f && nharderrors > 35)
                    && !(ipass > 2 && nharderrors > 39) && !(ipass == 4 && nharderrors > 30)) {
                    if (checkCRC12(decoded)) {
                        if (syncStats)
                            emitEvent(JS8::Event::SyncState { JS8::Event::SyncState::Type::DECODED,
                                                              Mode::NSUBMODE,
                                                              f1,
                                                              xdt2,
                                                              { .decoded = sync } });
                        ...
                        xsnr = std::max(
                            10.0f * std::log10(std::max(xsig / xbase - 1.0f, 1.259e-10f)) - 32.0f,
            ...
            for (int ipass = 1; ipass <= 4 && totalLdpcPasses < m_maxLdpcPasses; ++ipass) {
                ...
                if (ipass == 3)
                    std::fill(llr0Combined.begin(), llr0Combined.begin() + 24, 0.0f);
                else if (ipass == 4)
                    std::fill(llr0Combined.begin() + 24, llr0Combined.begin() + 48, 0.0f);
                ...
                if (m_enableLdpcFeedback && totalLdpcPasses < m_maxLdpcPasses) {
                    ...
                    int confident = 0;
                    int uncertain = 0;
                    js8::refineLlrsWithLdpcFeedback(llrPrimary,
                                                    cw,
                                                    m_llrErasureThreshold,
                                                    llrRefined,
                                                    confident,
        ...
        void js8_downsample(float const f0)
        {
            ...
            std::copy(ds_cx.begin() + ib, ds_cx.begin() + ib + RANGE_SIZE, cd0.begin());
            ...
            std::transform(cd0.begin(), cd0.end(), cd0.begin(), [factor](auto& value) {
                return value * factor;
            });
        }
        ...
        std::vector<Sync> syncjs8(int nfa, int nfb)
        {
            ...
            for (int i = ia; i <= ib; ++i) {
                ...
                for (int j = -Mode::JZ; j <= Mode::JZ; ++j) {
                    ...
                    if (auto const sync_value
                        = std::max({ compute_sync(0, 2), compute_sync(0, 1), compute_sync(1, 2) });
                ...
                sync.emplace(Mode::DF * i, Mode::TSTEP * (max_index + 0.5f), max_value);
            }
            ...
            auto const normalize = [sync = rankIndex.nth(rankIndex.size() * 4 / 10)->sync](
            ...
            for (auto it = syncIndex.begin(); it != syncIndex.end() && candidates.size() < NMAXCAND;
                 it = syncIndex.begin()) {
        ...
        float syncjs8d(int const i0, float const delf)
        {
            ...
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 7; ++j) {
                    if (auto const offset = 36 * i * Mode::NDOWNSPS + i0 + j * Mode::NDOWNSPS;
        ...
        std::vector<std::complex<float>> genjs8refsig(std::array<int, NN> const& itone,
                                                      float const f0)
        {
        ...
        void subtractjs8(std::vector<std::complex<float>> const& cref, float const dt)
        {
    ...
    public:
        ...
        DecodeMode()
        {
            ...
            std::transform(filter.begin(),
                           filter.end(),
                           filter.begin(),
                           [factor = 1.0f / Mode::NMAX](auto value) { return value * factor; });
        ...
        std::size_t operator()(struct dec_data const& data,
                               int const kpos,
                               int const ksz,
                               JS8::Event::Emitter emitEvent)
        {
            ...
            auto const ddCopy = [](auto const begin, auto const end, auto const to) {
            ...
            if ((JS8_RX_SAMPLE_SIZE - pos) < sz) {
                ...
                ddCopy(std::begin(data.d2) + pos,
                       std::begin(data.d2) + pos + firstsize,
                       dd.begin());
                ddCopy(std::begin(data.d2),
                       std::begin(data.d2) + secondsize,
                       dd.begin() + firstsize);
            } else {
...
class Worker : public QObject
{
    ...
    class Impl
    {
        ...
        struct DecodeEntry
        {
            ...
            DecodeEntry(std::in_place_type_t<DecodeModeType>, int mode, int& kpos, int& ksz) :
                decode(std::in_place_type<DecodeModeType>), mode(mode), kpos(kpos), ksz(ksz)
            {
            }
        };
        ...
        template <typename ModeType> DecodeEntry makeDecodeEntry(int shift, int& kpos, int& ksz)
        {
            return DecodeEntry(std::in_place_type<DecodeMode<ModeType>>, 1 << shift, kpos, ksz);
        }
        ...
        void operator()(::JS8::Event::Emitter emitEvent)
        {
            ...
            for (auto& entry : m_decodes) {
                if ((set & entry.mode) == entry.mode) {
                    std::visit(
                        [&](auto&& decode) {
                            sum += decode(m_data, entry.kpos, entry.ksz, emitEvent);
                        },
                        entry.decode);
                }
            }
...
public slots:
    ...
    void run()
    {
        ...
        while (true) {
            ...
            (*impl)([this](::JS8::Event::Variant const& event) { emit decodeEvent(event); });
        }
    }
};
} // namespace JS8
            ...
            int* const tones)
{
    ...
    for (int i = 0, j = 0; i < 12; i += 4, j += 3) {
        std::uint32_t words = (alphabetWord(message[i]) << 18)
            | (alphabetWord(message[i + 1]) << 12) | (alphabetWord(message[i + 2]) << 6)

...
public:
    struct Key
    {
        ...
        {
            return mode == other.mode && freqBin == other.freqBin && dtBin == other.dtBin
    ...
    explicit SoftCombiner(bool enabled, bool runSelfTest = true) : m_enabled(enabled)
    {
        if (!m_enabled) {
                     ...
                     std::chrono::seconds ttl)
    {
        ...
        qCDebug(decoder_js8) << "soft-combining repeats" << it->repeats << "mode" << key.mode
    ...
    void markDecoded(Key const& key)
    {
        ...
        bucket.erase(
            std::remove_if(bucket.begin(),
                           bucket.end(),
                           [&key](Entry const& entry) { return entry.signature == key.signature; }),
            bucket.end());
    ...
    void flush(std::chrono::seconds ttl)
    {
        ...
        for (auto it = m_entries.begin(); it != m_entries.end();) {
            ...
            bucket.erase(std::remove_if(
                             bucket.begin(),
                             bucket.end(),
                             [now, ttl](Entry const& entry) { return now - entry.lastSeen > ttl; }),
                         bucket.end());
...
private:
    ...
    static Bucket::iterator findEntry(Bucket& bucket, uint32_t signature)
    {
        ...
        return std::find_if(bucket.begin(), bucket.end(), [signature](Entry const& entry) {
    ...
    static uint32_t signature(std::array<float, N> const& llr0, std::array<float, N> const& llr1)
    {
    ...
    static void maybeRunSelfTest()
    {
        ...
        std::call_once(once, []() {
            ...
            auto countMatches
                = [](std::array<float, N> const& llr, std::array<float, N> const& reference) {
                      int matches = 0;
            ...
            qCDebug(decoder_js8) << "soft-combining self-test: A matches" << matchesA << "B matches"
                                 << matchesB << "combined matches" << matchesCombined << "repeats"
                                 << second.repeats;
        });
    }

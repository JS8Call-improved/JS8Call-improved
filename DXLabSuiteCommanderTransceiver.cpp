        ...
        while (tx != on && DriftingDateTime::currentMSecsSinceEpoch() - start < 1000) {
            ...
            if (0 == reply.indexOf("<CmdTX:")) {
                ...
                if ("ON" == state) {
                    ...
                    throw error { tr("DX Lab Suite Commander sent an unrecognised TX state: ")
                                  + state };
                }
            } else {
                TRACE_CAT("DXLabSuiteCommanderTransceiver", "get TX unexpected response" << reply);
                throw error {
                    tr("DX Lab Suite Commander didn't respond correctly polling TX status: ")
                    + reply
                };
            }
            if (tx != on)
...
void DXLabSuiteCommanderTransceiver::poll()
{
    ...
    if (state().split()) {
        ...
        if (0 == reply.indexOf("<CmdTXFreq:")) {
            ...
            throw error {
                tr("DX Lab Suite Commander didn't respond correctly polling TX frequency: ") + reply
            };
        }
    }
...
QString DXLabSuiteCommanderTransceiver::command_with_reply(QString const& cmd, bool no_debug)
{
    ...
    while (!replied && --retries) {
        ...
        if (!replied && commander_->error() != commander_->SocketTimeoutError) {
            ...
            throw error { tr("DX Lab Suite Commander send command \"%1\" read reply failed: %2\n")
                              .arg(cmd)
                              .arg(commander_->errorString()) };
        }
    }
    ...
    if (!replied) {
        ...
        throw error {
            tr("DX Lab Suite Commander retries exhausted sending command \"%1\"").arg(cmd)
        };
    }

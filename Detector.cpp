...
constexpr std::array LOWPASS {
    0.000861074040f,  0.010051920210f,  0.010161983649f,  0.011363155076f,  0.008706594219f,
    0.002613872664f,  -0.005202883094f, -0.011720748164f, -0.013752163325f, -0.009431602741f,
    0.000539063909f,  0.012636767098f,  0.021494659597f,  0.021951235065f,  0.011564169382f,
    -0.007656470131f, -0.028965787341f, -0.042637874109f, -0.039203309748f, -0.013153301537f,
    0.034320769178f,  0.094717832646f,  0.154224604789f,  0.197758325022f,  0.213715139513f,
    0.197758325022f,  0.154224604789f,  0.094717832646f,  0.034320769178f,  -0.013153301537f,
    -0.039203309748f, -0.042637874109f, -0.028965787341f, -0.007656470131f, 0.011564169382f,
    0.021951235065f,  0.021494659597f,  0.012636767098f,  0.000539063909f,  -0.009431602741f,
    -0.013752163325f, -0.011720748164f, -0.005202883094f, 0.002613872664f,  0.008706594219f,
    0.011363155076f,  0.010161983649f,  0.010051920210f,  0.000861074040f
};
} // namespace
...
Detector::Detector(unsigned frameRate, unsigned periodLengthInSeconds, QObject* parent) :
    AudioDevice(parent), m_frameRate(frameRate), m_period(periodLengthInSeconds), m_filter(LOWPASS)
{
    clear();
}
...
void Detector::resetBufferPosition()
{
    ...
    unsigned const msInPeriod = (now % 86'400'000LL) % (m_period * 1000);
    int const prevKin = dec_data.params.kin;

    dec_data.params.kin = qMin((msInPeriod * m_frameRate) / 1000,
...
qint64 Detector::writeData(char const* const data, qint64 const maxSize)
{
    ...
    size_t const framesAcceptable
        = (sizeof(dec_data.d2) / sizeof(dec_data.d2[0]) - dec_data.params.kin) * Filter::NDOWN;
...
unsigned Detector::secondInPeriod() const
{
    ...
    unsigned secondInToday((now % 86'400'000LL) / 1000);

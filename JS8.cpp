                ...
                std::array<int8_t, N>& cw)
{
    ...
    for (int iter = 0; iter <= BP_MAX_ITERATIONS; ++iter) {
        ...
        for (int i = 0; i < M; ++i) {
            for (
                int j = 0; j < 7;
                ++j) { // Fixed range [0, 7) to match Fortran's 1:7, could be nrw[j], or 7 logically
    ...
    if (checkCRC12(decoded)) {
        ...
        for (std::size_t i = 0; i < 12; ++i) {
            words[i] = (decoded[i * 6 + 0] << 5) | (decoded[i * 6 + 1] << 4)
                | (decoded[i * 6 + 2] << 3) | (decoded[i * 6 + 3] << 2) | (decoded[i * 6 + 4] << 1)
                | (decoded[i * 6 + 5] << 0);
        }
...
template <typename Mode> class DecodeMode
{
    ...
    {
        return [this]<Eigen::Index... I>(float const x, std::integer_sequence<Eigen::Index, I...>) {
            auto baseline = 0.0;
            auto exponent = 1.0;
                                 ...
                                 JS8::Event::Emitter emitEvent)
    {
        ...
        for (int ipass = 1; ipass <= 4; ++ipass) {
            ...
            if (ipass == 3)
                std::fill(llr0.begin(), llr0.begin() + 24, 0.0f);
            else if (ipass == 4)
                std::fill(llr0.begin() + 24, llr0.begin() + 48, 0.0f);
            ...
            if (nharderrors >= 0 && nharderrors < 60 && !(sync < 2.0f && nharderrors > 35)
                ...
                if (checkCRC12(decoded)) {
                    ...
                    xsnr = std::max(10.0f * std::log10(std::max(xsig / xbase - 1.0f, 1.259e-10f))
                                        - 32.0f,
    ...
    void baselinejs8(int const ia, int const ib)
    {
        ...
        auto const mapIndex
            = [ia, ib, last = size - 1](int const i) { return (i - ia) * last / float(ib - ia); };
...
public slots:
    ...
    void run()
    {
        ...
        while (true) {
            ...
            (*impl)([this](Event::Variant const& event) { emit decodeEvent(event); });
        }
    }
};
} // namespace JS8
...
Decoder::Decoder(QObject* parent) :
    QObject(parent), m_semaphore(0), m_worker(new Worker(&m_semaphore))
{
